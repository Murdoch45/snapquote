...
- **No observable change from the fix.** `ads_get_ad_accounts` on `978213371900828` is byte-identical to the pre-fix reading: ACTIVE, `is_ads_mcp_enabled: true`, `is_ads_mcp_disabled_reason: null`. ⚠️ **Could not determine what the fix was** — no API/MCP tool exposes the connector's token scopes or per-account allowed-actions, so a permissions change would leave no readable trace. Consistent with the 2026-07-21 root cause (MCP-side bug), which now has a fifth confirming data point.
- ✅ **Manual CSV load COMPLETED.** Live audience read: `delivery_status` **ACTIVE**, `operation_status` **200 "Normal"** (was 300 "Updating" hours earlier), `time_updated` 2026-07-21T04:45:40 unchanged. The Updating→Normal transition means Meta finished processing the list — **the seed is loaded and usable**; the MCP bug is an inconvenience for future top-ups, not a blocker.
- ⚠️ **Exact matched count not readable** — bounds pinned at the 1000/1000 display floor (same behaviour as ABM v0), so "1000" is a floor artifact, not a measurement.
- 📝 Minor: the audience *description* still reads "574 rows, 43 states" — stale metadata from creation, before the seed grew to 1,355. Cosmetic; worth editing in Ads Manager.
- **Future top-ups:** Ads Manager → Audiences → Seed v1 → Update → Add customers → upload CSV → map → Import & create (does not reset the campaign learning phase). Re-test signal: a *numeric* Meta error code replacing the null-coded `INTERNAL` would mean the server-side bug is fixed.
- Notion `3a432498-a1cb-815e-b3d5-c454b0d53fa2`.

### 2026-07-21 [Source: Claude Code] — ROOT CAUSE: Ads MCP audience-user upload is an MCP-side bug, not permissions/ToS
**Read-only investigation + one permitted single-row test upload. No spend.** Verdict: **(d) MCP server bug** on the `ads_update_custom_audience_users` path — NOT actions-disabled, NOT a missing write scope, NOT a Custom Audience ToS / business-verification gate. The "Actions allowed — dash" is a **red herring for this failure**.
- **Decisive: the same account + same audience + same customer-list data uploads fine via Ads Manager.** Live read of Seed v1 `120254923187940273` after the manual CSV upload: `delivery_status` **INACTIVE → ACTIVE**, `operation_status_code` **200 "Normal" → 300 "Updating"**, `time_updated` **2026-07-20T17:08:44 → 2026-07-21T04:45:40**. Meta's own docs make "accept the custom audience terms" a required step in that wizard, so a successful manual upload **proves the ToS is accepted** — and equally rules out the documented "recently linked account" provisioning delay and any verification gate.
- **Other MCP actions succeed through the same connector:** `ads_create_custom_audience` (twice) and `ads_delete_custom_audience` both returned success. A connector with no actions, or a token without write scope, could not have done that → eliminates (a) and (b).
- **The failure predates the disconnect/reconnect** — uploads were already failing while creates/deletes were succeeding, so a permission state that appeared at reconnect can't explain it. Post-reconnect account read is clean: ACTIVE, `is_ads_mcp_enabled: true`, `is_ads_mcp_disabled_reason: null`.
- **Error shape points inward:** `error_code: null, error_subcode: null` with generic `INTERNAL`, whereas real Meta rejections from this same MCP carry numeric codes (`2654` audience-create, `2446496` ad-create).
- **Cumulatively ruled out:** batch size (290/100/3/1), schema (8-field, PHONE-only, EMAIL-only), raw **and** correct pre-hashed SHA-256 values (so not the hashing layer), both `customer_file_source` values, two audiences, account standing, connector reconnect. No alternate add-users tool exists. `customer_consent: true` deliberately not set (false attestation for skip-traced contacts).
- **Test result: still fails** on the smallest correct-by-spec payload (1 row, `schema: ["EMAIL"]`).
- **Fix: none available API-side** — the fault is inside the MCP server. **Manual CSV is the working path and is already working.** To add the rest: Ads Manager → Audiences → Seed v1 → Update → Add customers → upload CSV → map → Import & create (Meta notes this does **not** reset the campaign learning phase).
- ⚠️ **Could not verify live:** no API/MCP tool reports the connector's per-account allowed-actions; `ads_get_ad_accounts` exposes only `is_ads_mcp_enabled`/`is_ads_mcp_disabled_reason` with no action-level breakdown, so the Business-Settings dash is neither readable nor changeable programmatically. It may matter later for campaign publishing; it does not gate audience writes today.
- Notion `3a432498-a1cb-819d-94e5-cdecdcf7a766`.

### 2026-07-20 [Source: Claude Code] — EXECUTION r3: seed maxed to 1,355; upload diagnosed (MCP-side, not ToS); Meta-ready CSV delivered
**Spend: ScraperCity +$19.9000 this round; $39.0239 committed cumulatively of the $40.17 ceiling → $2.1461 remaining vs the $1.00 floor. Origami untouched (530.15).** No lookalike, no campaign; ABM v0 / pilot / paused campaign untouched.
- **Seed 755 → 1,355 rows** (+600), 44 states, 1,125 with personal email. CSLB re-downloaded free → 2,617 candidates; untapped pool built by exclusion (recovered **955 already-traced fed names from all 21 saved runIds**, then excluded seed + corpus + franchise screen) → **1,603 untapped across 345 cities**. Traced 1,000 → surname+first+state+mobile gate → **600 kept = 60%, the best yield of any round**.
- **CA 71.5%** (969/1,355) — expected and accepted; geographic balance was explicitly dropped as a constraint this round in favour of size. Projects to **~675 matched** at Meta's ~50%, past the ~500 target.
- ⚠️ **Wallet lag caught in the act, worth recording:** right after the run the wallet displayed **$4.14** while committed math said **$2.15** — over-reporting by ~$2. Trusting it would have made two more chunks look affordable and breached the floor. It later settled to exactly $2.15. Same failure mode as the $149 incident; committed-cost is the only safe control. Guard halted at chunk 20 (chunk 21 would have hit $40.02 vs the $39.42 working ceiling).
- **Upload still fails** — `ads_update_custom_audience_users` returns `INTERNAL` on every attempt. Live checks: account `978213371900828` is **ACTIVE / MCP-enabled / queryable / has payment method / no disable reason**; **ABM v0 is ACTIVE and populated on the same account**, so writes demonstrably worked here on 07-10/13; **a pre-hashed single row also fails**, ruling out the MCP's normalise-and-hash layer. Now ruled out across two sessions: batch size (290/100/3/1), schema (8-field and PHONE-only), raw vs hashed, both `customer_file_source` values, and two different audiences.
- **Best evidenced hypothesis: an MCP-side exception, not a Meta policy rejection.** This error returns **`error_code: null / error_subcode: null`**, whereas real Meta rejections from the same MCP carry numeric codes (audience-create `2654`, ad-create `2446496`). ⚠️ Caveat: Custom Audience ToS acceptance is **not exposed by any available API read**, so it can't be positively excluded — if it is the cause, the manual CSV upload will surface the acceptance prompt, settling it at zero cost. `customer_consent: true` deliberately **not** set — it attests consent was obtained, which is false for skip-traced contacts.
- **Deliverable: `C:\Users\murdo\Downloads\snapquote-seed-v1-meta-upload.csv`** — 1,355 rows + header, 93,548 bytes, Meta's `email,phone,fn,ln,ct,st,zip,country` codes, raw values (Ads Manager hashes client-side). Validated: 0 malformed, 1,355/1,355 valid E.164 phones, all US. Click path: Ads Manager → Audiences → Seed v1 `120254923187940273` → Edit/Add customers → upload CSV.
- **Cleanup:** throwaway diagnostic audience `120254923216750273` (Seed v1b, empty, 0 ad sets) **deleted**. Remaining: Seed v1 (empty), ABM v0, Pilot.
- Notion `3a432498-a1cb-81dd-9026-d54e9055805b`.

### 2026-07-20 [Source: Claude Code] — EXECUTION r2: seed expanded 574 → 755 (44 states, CA 49%); sequential Origami ~32% cheaper; upload still blocked
**Spend: ScraperCity +$2.6268 (cumulative $19.1239 of the $38.17 allowance; balance $22.05, $3.00 floor never approached). Origami 687.8 credits (balance 530.15, stopped 30.1 above the 500 floor).** No lookalike, no campaign; ABM v0 / pilot / paused campaign untouched.
- **The 112 set-aside rows were reconstructed, not assumed.** Last run's file was deleted with the PII scratch, so I re-downloaded the 12 CA runs free and re-applied the gates directly to the traced rows (each carries the fed licence name in `Input Given` *and* the returned person, so the surname gate re-runs without rebuilding the 160 MB CSLB files). **Re-derived 349 CA survivors — exactly reproducing last run's 349**, an independent confirmation the pipeline is deterministic. → 91 licence-CA + 21 existing-CA = **112**, matching exactly. One runId 502'd and succeeded on retry; the rescued copy overlapped the direct download, so rows are deduped on the echoed input (650 raw → 600 unique).
- **Sequential Origami (efficiency lesson #1): 226 rows, 687.8 cr = 3.04 cr/row vs last run's ~4.5 parallel — a ~32% improvement.** ⚠️ But **the 1.65 cr/row scope rate did NOT hold**: only run 1 hit it (1.78), then cost degraded to 2.40 → 7.02 → 4.53. "Sequential is ~3× cheaper" is too strong — the real finding is that per-run cost is highly variable and early runs are the cheap ones. Floor guard stopped the 5th run correctly.
- **Extraction (lesson #2): 132 usable of 226 = 58%** with the blob-aware extractor reused up front. Nothing lost to schema drift — the dominant loss was **75 rows where Origami found no owner name at all**, not parsing failure. (Last run: 67% after hardening vs 20/277 naive.)
- **New rows' geography exactly on target — 18 states, zero CA/FL/TX:** CO 22 · WI 19 · NE 14 · ID 12 · UT 10 · MN 10 · NY 9 · IA 7 · RI 7 · SD 6. Traced 132 → state gate → **72 kept (55%)**. runIds `ZDVrgzKNl9vXkqgge`, `MdMKlx7QNBSBIDCQt`, `yMWg2j7YQDr261DsQ`.
- **Expanded seed: 755 rows** (574 + 91 + 21 + 69 after 3 dedupe drops), **44 states**, 648 with personal email, 754 with zip.
- ⚠️ **CA share 49.0%**, above the ~45% guideline — the national yield was 69 survivors, not the 250–300 hoped for, because the Origami credit floor was reached. **Stopped growing at 755 deliberately:** Origami is exhausted, so the only remaining supply is CA licence rows (~1,600 candidates, ~$19 headroom), and the ~145 rows needed to reach 900 would push CA to ~57%. Size-vs-balance is now Murdoch's call, one short run away either way.
- **Upload retry (single permitted attempt): still `INTERNAL`** on `ads_update_custom_audience_users` for audience `120254923187940273`. Not retried further. Audience empty; payload ready.
- Notion `3a432498-a1cb-8139-97cb-d6237ffc66ff`.
...
- **Structural finding:** `FullBusinessName` holds the person only when the licence is in their own name (~22%); the other ~78% trade under a DBA and their owner exists **only in the Personnel file** — the Master→Personnel join is mandatory. Personnel title is literally **"Sole Owner" on 2,367** target records, confirming owner (not hired qualifier).
- **Ceiling (live):** C-27 12,622 all → 11,988 active → 5,606 sole-owner → 3,222 WC-exempt; C-53 2,962 → 2,829 → 1,007 → 647. **Usable with a real owner name: 2,617** (Landscaping 2,257 + Pool 360).
- **Test n=100, $1.99:** returned a person 96 · wireless mobile 82 (85%) · personal email 73 (76%) · in-CA 86 (90%) · exact city 57 · **WRONG PERSON 24 (25%)**. **LOOSE rate (state gate only) 73% → STRICT rate (surname+first match + CA + mobile) 59%.**
- ⚠️ **Biggest finding, unplanned:** ScraperCity fuzzy-matches on **first name alone** and returns a different human **in a nearby same-state city** — so these impostors **pass the state gate**. The 2026-07-13 state-cross-check finding stands but is **incomplete**: state matching alone would have waved through 14 wrong people and overstated the rate as 73%. The fix exists only on this track — the licence supplies an **authoritative surname**, enabling a surname-compatibility gate (Zuniga-Cruz→Zuniga and Felix→Delatrenidad Felix count as matches; different surnames dropped). **On the Origami path that gate is impossible** — the fed name is itself wrong ~20% of the time, so contamination is undetectable.
- **Survivor verification (free web, 15 of the 59):** owner tie CONFIRMED 10 / PLAUSIBLE 4 / UNVERIFIABLE 1 / **CONTRADICTED 0**; real+active 15/15; **SOLO 14 + SMALL 1 = 100% solo/small**; trade correct 15/15. The WC-exemption filter genuinely selects one-person operations.
- **vs Origami:** name correctness ~100% by construction vs 72% (20% flat wrong); end-to-end **59% vs ~46%**; impostors detectable vs not; sourcing $0/no credits vs credits; but CA-only + licensed-trades-only vs national/any-trade.
- **Economics:** remaining $41.17 buys ~2,165 lookups → ~1,275 clean owner mobiles → **~575–700 matched** at 45–55%, zero Origami credits — roughly the whole seed target from one free source.
- **Three honest limits:** (1) a 100%-CA seed teaches Meta "California landscaper" — blend with the 361 national records + an Origami run; (2) 86% landscaping / 14% pool, and pressure washing + tree are **not licensed in CA** (CA also bars the WC exemption for D-49 tree); (3) pool is a recurring-contract trade — take a slice, not all 360.
- **FL DBPR: could not verify live** — public-records page and a "datamart" portal exist but no direct free bulk-download link found this pass.
- Committed: [scripts/cslb_source.py](scripts/cslb_source.py) — downloader + filter + Master→Personnel join, with the name-parsing bugs this test exposed fixed ("MC GEE" re-glue, junk-name rejection) and the `surname_compatible()` impostor gate; self-tested. Notion `3a332498-a1cb-81da-9846-c21b233baf50`.

### 2026-07-20 [Source: Claude Code] — PLAN: clean solo-operator lookalike seed — probe + quality screen + projected ~600–750 matched (0 credits, $0)
**Plan only, awaiting Murdoch's approval.** Probe spend **0 Origami credits** (verified 3× post-settlement), **$0 ScraperCity**, **$0 Meta**. No full sourcing run, no enrichment, no Meta writes; campaign untouched. Live balances: Origami **2,944.25 cr**, ScraperCity **$43.16** Starter (→ 2,271 max lookups). Existing corpus re-derived live: 1,123 unique, **361 target-trade** (Lawn 252, PW 81, Tree 27, Pool 1, **Junk 0**), 280 of them with a mobile.
- **PROBE (the gate):** one agent, Phoenix + Tampa, asked for small independent owner-operators excluding chains. **Requested 25, delivered 25 — Pool 8/8, Junk 8/8, Lawn 3/3, Pressure 3/3, Tree 3/3**; 100% owner-name and website fill. ⚠️ **Cost 0 credits** — the agent returned owner names as static research, not paid `code` cells (`credits.lifetimeUsed`=0). This **contradicts the 1.78 cr/contractor model** from 2026-07-13; both models carried through the numbers. Run ended `incomplete` at 12/30 steps but still materialized all rows.
- **Quality gate on all 25** (free web verification, 44 agents, every CONFIRMED adversarially refuted): owner **net-correct 18/25 (72%)**, wrong-person 5/25 (20%), **trade correct 25/25**, **SOLO+SMALL 25/25** (16 SOLO / 9 SMALL), 1 franchise. Per trade — Lawn 3/3, Tree 3/3, Pressure 2/3(+1 plausible), **Pool 6/8**, **Junk 4/8 with 3 wrong**.
- **Verdicts: Pool Service KEEP** (depth + cleanliness confirmed). **Junk Removal DROP** — worst owner accuracy of any trade and the most franchise-saturated candidate (College HUNKS 370+, 1-800-GOT-JUNK ~138), and it contributes 0 rows today.
- **Key insight:** every business was solo/small because the *sourcing prompt* constrained it. **Prompt-level constraint at source beats any post-hoc size filter.**
- **Honest admission:** a reliable free small-business SIZE filter does not exist at scale. REJECTED with evidence — Google review count (`userRatingCount` forces the **Enterprise** SKU, only **1,000 free/mo**, field mask bills at the highest tier, and Maps ToS forbids retaining Places content beyond `place_id`; low count ≠ solo), BBB/D&B/SoS employee count (field doesn't exist), an Origami AI size column (~1,100cr to launder a guess), website page-count/owner-voice/phone-extension heuristics (refuted; score-only, never drop).
- **Screen order (free until step 5):** scope gate at source → dedupe vs the 1,123 (normalized company → person → registrable domain) → franchise/roll-up brand exclusion → require 2-token owner name → **only then** ScraperCity `"First Last; City, ST ZIP"` (cap on committed cost) → **state cross-check** (traced state ≠ business state → drop; keeps ~64%, lifts precision ~55%→~80%) → upload.
- **Best authoritative free solo signal found:** state contractor-license bulk files (CA CSLB, FL DBPR) filtered to Active + Sole Ownership + workers'-comp exemption — a perjury-backed "no employees" declaration, $0, whole-population. Coverage-limited (CA bars the exemption for D-49 tree; pressure washing/junk unlicensed). Better as a **$0 parallel source** than a screen — the licensee is the legal owner by construction, killing the 20% wrong-person class.
- **Numbers:** Scenario A (1.78cr, credits bind) 1,653 sourced → 1,208 traced ($22.96) → **427–518 total matched**. Scenario B (0cr, dollars bind) 2,740 sourced → 2,003 traced ($38.06) → **609–741 total matched**. Skipping the state gate would give 766–936 matched but ~420 of them wrong people — **take the gate; a clean 600–750 beats a dirty 900.**
- **Lookalike:** build a **NEW** seed audience (ABM v0 is 28% unclassifiable + Painting 284 + Roofing 115 — not a clean seed); stack phone+email+name+geo+country; Meta floor is **100 matched**; use **2–5%, start 3%, not 1%** (ratio is a share of the location population); duplicates don't up-weight; refresh monthly via Update to preserve the learning phase.
- ⚠️ **Compliance flagged for Murdoch:** Meta Customer List terms require rights to use uploaded data; skip-traced third-party mobiles for people with no SnapQuote relationship is a different posture from a customer list. Business decision, surfaced not buried.
- Reference implementation committed: [scripts/seed_screen.py](scripts/seed_screen.py) (designed, not run at scale; self-tested incl. the "s-**tree**-t" false-positive fix). Notion `3a332498-a1cb-81b3-bceb-c893b7de1b85`.

### 2026-07-20 [Source: Claude Code] — Company/trade backfill for the 372 company-less: Notion searched exhaustively, 57 resolved ($0, read-only)
**$0 spent.** No Prospeo (cancelled — never contacted), no paid API, no enrichment, no skip-traces, no Meta writes; paused campaign untouched. **Result: 57 of 372 resolved (15.3%) — Notion 0, email-domain keyword 47, free web lookup 10.**
- **Notion does NOT contain these contractors — proven, not asserted.** Ran 8+ query variations and **fetched 22 pages** (every `Lead-gen run` page 2026-05-26→06-15 plus the 102k-char Distribution & Partnerships hub). Harvested **217 unique lead-level records** + 179 culled company names from the **7 pages that carry per-lead detail** (the six `Landscaping 2 email pull — page N` children + the hub). The other **15 pages are aggregate-only** — per-campaign counts, stop reasons, credit deltas, zero individual leads.
- **Decisive cross-reference:** of the **139** unique person-names in Notion's lead-level records, **120 match the with-company cohort, 19 match nobody, exactly 0 match the 372.** A parser-independent check (all 634 name-like tokens in the fetched text) agreed: 130 overlap with-company, **0** overlap the 372.
- **Why:** Notion's per-lead logging belongs to the early manual claude.ai pulls (2026-05-21→25) — the same population that survives in transcripts/disk. The 372 come from the later skill-driven runs (05-26→06-15), and the skill logged **only aggregates**. The company field was dropped at source by design (skill kept name + verified email only). Corroboration: the 372's domain-resolved trades are overwhelmingly Roofing + Tree Service — exactly the June campaigns.
- **Free-signal fallback:** **0 of 372 have a work email** (that's why they're company-less), so skip-traced personal emails were used. 54 have no email at all; 166 have only consumer/ISP domains; 152 have a non-consumer domain. All **200** non-consumer domains were web-checked free (4 batches): **61 genuine contractor domains, 139 not**.
- ⚠️ **Method note worth keeping:** naive domain substring matching gave 50 hits but was inflated by "s-**tree**-t" false positives; a strict boundary rule cut it to 8 but wrongly killed legitimate compounds ("raines**tree**care"). Correct method = strip known container words (street/washington/greensboro…) then match loosely → **47**. Web verdicts override keyword (a "weatherproofing" domain is building-envelope restoration, not roofing).
- **The 315 unresolved, with a reason each:** 166 consumer-mailbox/ISP only · ~95 non-consumer domain that the web check found is not a contractor/dead · 54 no email of any kind. Nothing free remains; recovery would need a paid provider.
- **Updated trade mix across all 1,123:** Unclassifiable 315 (28.0%) · Painting 284 (25.3%) · Lawn/Landscaping 252 (22.4%) · Roofing 115 (10.2%) · Pressure Washing 81 (7.2%) · Other-with-company 46 (4.1%) · Tree 29 (2.6%) · Pool 1 · Fence 0. Roofing rose 88→115 and Tree 1→29 via the domain/web resolution. A tier-3 loose rule (Floralawn/TurfGator/Greenscapes/*Coatings) cut has-company-but-unclear from 72→46.
- Notion `3a332498-a1cb-8189-8995-d20e30608ca6`. PII scratch deleted at task end; scripts not committed (they read personal transcript stores).

### 2026-07-20 [Source: Claude Code] — ✅ FULL LIST RECOVERED: 1,123 unique contractors ($0, read-only)
**$0 spent** (ScraperCity wallet unchanged at $43.16, live read) — GET/download endpoints only, never a POST to `/scrape/*`; no enrichment; no Meta writes; paused campaign untouched. Recovered the contractor list from every source.
- **Result: 1,123 unique contractors** — the 1,059 cohort came back at **1,055 unique names (1,057 of 1,059 rows)**; disk added **68** never-traced extras.
- **⚠️ Corrects two prior beliefs (flagged, prior entries not edited):** (1) "ScraperCity results expire after 48h" is **wrong** — all 23 runIds returned `SUCCEEDED` and downloaded fine 7–10 days on, **0 dead**. (2) My own 2026-07-13 "only 514 recoverable" was **transcripts-only scope** and is superseded.
- **Source 1 — ScraperCity (1,055 unique):** `GET /api/v1/runs` returns **0 runs**, but that's a listing limit, not deletion — the endpoint is time-windowed and **caps at 168h** (requested `hours=8760`, got back `hours_requested:168`). Downloads by runId work regardless. 23 runs pulled: 310-batch **310/310 exact**, 749-batch **747** (the 2-row shortfall is original — run log recorded `submitted=749/downloaded=747`), A/B Phoenix 49 (separate cohort, excluded).
- 🔑 **runId `VHcZB0Jt6vhP12Ihn` (147 rows) exists in NO transcript — only in the 2026-07-10 Notion entry.** Recording runIds to Notion is what made full recovery possible; without it we'd be 147 short.
- **Source 2 — transcripts (515 unique):** 533 raw rows; re-confirms only the agent-mode root holds `add_leads_to_campaign_or_list_bulk` payloads (`~/.claude/projects` has none). **514 of 515 also appear in ScraperCity** — independent validation that the traced set is a superset.
- **Source 3 — disk (237 unique, 67 found nowhere else):** working dir `C:\Users\murdo\Snapquote-leadgen` (enriched.json 102, c1_batch2 72, c2_enriched 69, subagent4-state/state.json 56, keepers.json 142) — 441 raw rows. The 67 were never skip-traced.
- **Identity completeness:** 609 have the original work email · **899 have a skip-traced mobile** · 890 a personal email · 372 are ScraperCity-only (name + skip-trace data, no company/work email — ScraperCity rows carry `Input Given` but no company).
- **Gap vs 1,059, evidenced:** only **2 rows** truly lost (never returned by ScraperCity at submission) + 2 name-collisions collapsed in dedupe. Caveat: dedupe is by normalized full name, so nickname variants may shift the count slightly.
- **Trade mix** (the 751 with a company name): Painting 276 (36.8%), Lawn/Landscaping 233 (31.0%), Roofing 88 (11.7%), Pressure Washing 80 (10.7%), Other 72 (9.6%), Tree 1, Pool 1 — differs from 2026-07-13 because disk added painting-skewed records.
- Notion `3a332498-a1cb-810e-9a96-f403ffef2bf6`. Recovered list held in scratchpad and deleted after reporting; recovery scripts not committed (they read personal transcript stores) — the reusable recipe is documented above.

### 2026-07-13 [Source: Claude Code] — Trade segmentation of the recovered list + ⚠️ only 514 of 1,059 still recoverable ($0, analysis only)
**$0 spent.** No Meta writes (one read-only `ads_get_custom_audience`), no Origami/ScraperCity calls, paused campaign `120254670892560273` untouched. Segmented the recovered contractor list by trade so Murdoch can pick lookalike-seed trades.
- **⚠️ LIVE CONTRADICTS PRIOR ENTRIES:** prior docs/Notion say 1,059 leads are recoverable from transcripts; **live re-run recovers only 514 unique** (533 raw, 7 campaign IDs vs 10 before). The `~/.claude/projects` root now yields **0** lead payloads (1,440 matching lines are `attachment` records quoting the tool name; **0 lines contain `first_name`**) — leadgen transcripts rotated since 2026-07-09. The other ~545 survive **only as hashed identifiers inside Meta** and are NOT recoverable as a source list. Prior entries flagged historical-but-superseded, not edited.
- **Field shape verified live:** payload nests under `input.params`; leads carry `first_name`/`last_name`/`email`/`company_name` only — no city/state/trade field.
- **Trade mix (n=514; strict name/domain → campaign-inferred):** Lawn/Landscaping 205→**220 (42.8%)**; Painting 99→**113 (22.0%)**; Roofing 88→**97 (18.9%)**; Pressure Washing/Exterior 80→**83 (16.1%)**; Tree 1; **Pool 0; Fence 0**; Other/Ambiguous 41 (8.0%)→0.
- **Campaign purity 95–100% single-trade** (roofing 86/86, painting 88/88 & 11/11, lawn 75/75, 43/43, 87/88, pressure-wash 80/85) — leadgen built one campaign per trade, so campaign membership beats company-name keywords; used to resolve the 41 residuals. Zero pool/fence + only 1 tree ⇒ the 3 lost campaigns were almost certainly fence/pool/tree, so this sample is campaign-biased and is NOT the mix of the full 1,059 in Meta.
- **Seed sizing:** roofing is 1 of 10 original campaigns ≈ **~9–10% of the full 1,059** (~950 non-roofing) — excluding it costs far less than the 18.9% sample share suggests. Within the recoverable 514: non-roofing **417 (81.1%)**; owner-operator core (lawn+PW+tree, no painting) **304 (59.1%)**.
- **Matched (estimate only):** live audience `120254639160170273` still pinned **1000/1000**, ACTIVE/Normal, updated 2026-07-13 → matched is >100 (LAL probe proved ≥100) and under Meta's ~1,000 display floor. Meta exposes no per-record match status; non-roofing ≈ ~90% of whatever total matched is.
- **Recommendation:** seed the lookalike off **ABM v0 as-is** — ~9–10% roofing contamination beats halving the seed by rebuilding from the only-514 recoverable rows. Do trade filtering in the **scale run**, where Origami tags trade at source for free. Growing toward ~1,000 matched stays the higher-value move.
- Notion: `3a332498-a1cb-814a-8bf6-f4045f6f1c2e` (Distribution & Partnerships, aggregates only). PII scratch deleted at task end; recovery/classification scripts deliberately NOT committed (they mine personal transcript stores).

### 2026-07-13 [Source: Claude Code] — Pipeline DESIGN CHALLENGE: free filters + better-source study ($0, plan only)
**$0 spent** — free re-reads of the two paid A/B runs (`hmymOzXfQJQzc2K9g`/`nTFoIMG2KXkziG7UY`) + free web research + ScraperCity docs. Live campaign `120254670892560273` untouched. Answered Murdoch's 5 questions on whether a cheaper/cleaner owner-mobile pipeline exists before committing ~2,944 Origami credits.
- **Q1 name×address combined query:** NO such input exists. ScraperCity `/scrape/skip-trace` has 4 *independent* lanes (`name`,`email`,`phone_number`,`street_citystatezip`); the only name+location mechanism is the string `"Name; City, ST"` — which Path A already used. Adding the street address = Path B (already lost ~4×). Nothing new to build. Only sub-lever: ZIP granularity in the string.
- **Q2 free STATE cross-check filter (quantified on the 22 name records):** drop returned `Address Region` ≠ business state → keeps 14/22 (64%), lifts kept-precision from ~50–59% to **~80%**. ~7 of the 8 dropped were right-name→WRONG same-name person in another state (e.g. a CONFIRMED Phoenix owner's name returned a Bakersfield CA resident). Volume removed is ~⅞ junk → near-free precision. Use ~60% keep when metros are geo-targeted.
- **Q3 pre-spend filters (30-gate sample):** drop no-name (~2/30) + national/franchise (~5/30) + confidence<medium → keeps ~65–75% of named; `high` alone is NOT clean (≥1 WRONG tagged high).
- **Q4 better source:** biggest win is the two free FILTERS (full scale, $0), not a new source. Real source upgrade worth layering: **contractor's own website About/Team page** (grounded name via the URL Origami already returns; all states/trades; ~⅓–½ coverage) + **CA CSLB / FL DBPR free bulk license personnel files** (authoritative, licensed trades only). BBB / SoS member filings (disclosure states) / county biz-license = free per-record cross-checks. Drop GBP / BuildZoom / free D-U-N-S for owner identity.
- **Q5 recommended design:** Origami source → free website-About name-upgrade → **GATE α** (drop no-name/national, conf≥med, free) → dedupe → skip-trace `"Name; City, ST ZIP"` → **GATE β** (state cross-check, free) → additive upload to ABM v0 `120254639160170273`. Conservative funnel: 2,944cr → ~1,600 new → ~1,000 skip-traced (~$18–20) → ~280 new matched (**~220 good / ~60 noise, ~79% clean** vs baseline ~600 matched / 58% clean). Trades ~130 good to cut ~190 noise — right call for a Reach/CTR audience; website-About upgrade buys good volume back at $0. Binding constraint stays Origami credits, not dollars.
- **Validation:** $0 first (website-name vs Origami-name vs gate on existing 30, free web). Then one **<$3** paid test: ~120 fresh pre-filtered contractors skip-traced with `"Name; City, ST ZIP"` (~$2.40) to confirm GATE β precision beyond n=22, + ZIP-appended re-run of the 8 out-of-state names (~$0.16).
- Notion: `39c32498-a1cb-81dc-a552-f22a3449fc2e` (Distribution & Partnerships, aggregates only). No PII in repo/Notion; A/B + Phoenix scratch deleted at task end.
...
**Result (150 submitted, seed 1059, aggregates):** 141/150 (94%) returned a usable contact; **130/150 (87%) have ≥1 mobile/wireless phone** (vs Prospeo 80%); **119/150 (79%) have ≥1 personal/free-mail email** (Prospeo returned none — work-only); 135/150 (90%) got a NEW FB-matchable identifier. Precision caveat: only 74/141 hits (52%) are confirmably same-state as the business (skip-trace name+location matching returns some same-name/out-of-state people; Prospeo's LinkedIn-identity match is more precise per-record). Mobile recency strong (89 best-mobiles last-reported 2026). Cost verified live: $0.0199/result on the 3-record test before the batch; wallet decremented $2.05 (misses appear uncharged — ~103 billable of 150; `/runs` quotes $2.985 but wallet is ground truth).

**Recommendation (Gate — next spend needs approval):** trace the rest with ScraperCity — wins on cost (~$15 for all 1,059 vs Prospeo $50–100+/mo) and adds personal emails Prospeo lacks; accept ~50% right-person precision as tolerable noise for hashed Meta matching, uploading the most-recent Wireless number + one free-mail email stacked with name/geo into ABM v0. Keep Prospeo only for use cases needing verified identity (not ad matching). **Stopped at gate — did not trace remaining ~909; only the $0.02 skip-trace endpoint called; PII scratch deleted (results re-downloadable free from ScraperCity 48h via runIds `46C4K2MUr8f2ZQZMQ` / `VHcZB0Jt6vhP12Ihn`). Runner not committed to the app repo (marketing tooling, reads PII scratch) — per Origami/leadgen precedent.**

**Confirmed live from docs (app.scrapercity.com/api-docs, in-browser).** Endpoint `POST /api/v1/scrape/skip-trace`, $0.02/result; inputs name[]/email[]/phone_number[]/street_citystatezip[] + `max_results` (default 1); name[] accepts `"First Last; City, ST"`. Balance `GET /api/v1/wallet`; per-run cost `GET /api/v1/runs` (cost_usd/handled). Async POST→poll status→download JSON, 48h. 100/min, 10 concurrent. **Key finding:** People Finder's price line lacks the "FREE if not found" clause that Mobile Finder ($0.25) and Email Finder ($0.05) both carry → assume misses charged; 3-record test resolves it empirically.

**Setup ($0).** Re-recovered 1,059 leads (same method/count as Phase 0). Random 150-record sample, **seed=1059**. Pulled business city/state for the sample via free Pro-SKU Places (150 calls, $0; 116–117/150 got geo, 34 name-only). Built a runner with money guardrails enforced in code: only the skip-trace endpoint constructable, `max_results` pinned to 1, persistent ledger caps cumulative submitted ≤150 across runs and refuses phase re-charge, worst case ≤$3.00; key from gitignored `.env` (`SCRAPERCITY_API_KEY`, never hardcoded); verified offline that it aborts with no key.

**Stopped at end of Phase 1.** Awaiting Murdoch's API key before the 3-record test → ≤150 batch. Runner script kept in session scratch (not committed to the app repo — marketing tooling stays out, per Origami/leadgen precedent; reads PII scratch, deleted at task end).

### 2026-07-10 [Source: Claude Code] — Phase 0 EXECUTED ($0 total): ABM v0 audience (1,059 recovered contractor owners) live on Meta; LAL probe confirms ≥100 matched; Prospeo mobile test 8/10; stopped at Gate A

**What ran (2026-07-09 → 07-10 PT, per the approved "Cheap ABM Rethink" plan; full detail in Notion child page `39932498-a1cb-8140-b7d2-ed9203bb6299` under Distribution & Partnerships).** (1) Re-extracted the **1,059** owned leads from local transcripts (both roots: `AppData\Roaming\Claude\local-agent-mode-sessions` AND `~/.claude/projects/C--Users-murdo-Snapquote-leadgen/` — the second root holds the subagent transcripts; single-root pass finds only 514). (2) Geo-join via Places Text Search (New), Pro-SKU-only field mask verified live pre-run: **847/1,042 companies (81%) got city/state/zip; 1,043 calls total, all inside the 5,000-free/mo Pro cap — $0, 0 errors.** (3) Uploaded to NEW audience **`120254639160170273` "ABM v0 — Recovered contractor owners"** (CUSTOM, PARTNER_PROVIDED_ONLY, EMAIL+FN+LN+CT+ST+ZIP+COUNTRY, PHONE blank): **1,059 received, 0 invalid** in 4 batches; pilot audience untouched. (4) Probes: `operation_status 200 "Normal"`, **`delivery_status ACTIVE`**, bounds pinned 1000/1000 (below display floor, expected); **1% lookalike probe PASSED** (probe LAL `120254658125530273` built to Normal/ACTIVE → seed ≥100 matched per Meta's documented 100-member LAL minimum; probe deleted after reading). (5) **Prospeo free mobile test: 8/10 verified US mobiles** on randomly-sampled owners (only_verified_mobile, name+company+website inputs); 7/8 distinct from the company HQ line; 72 of 100 free credits used (8×9cr discounted re-reveal rate), $0 cash.

**Spend: $0 total.** **Gate A state:** ABM v0 is a live, deliverable, LAL-seedable audience of ≥100 (<~1,000) matched contractor owners. Recommendation logged in Notion: 8/10 mobile hit rate justifies Phase 1 (one Prospeo month ~$49–99, cancel after → several-hundred-matched upgrade) before the Gate-B Reach ad; running the ad on v0 as-is is the $0 alternative. Use or refresh the audience by ~2026-08-09 (30-day Outdated flag). PII stayed scratch-only and was deleted at task end.

### 2026-07-09 [Source: Claude Code] — Cheap ABM rethink: 1,059 old Prospeo→Instantly leads RECOVERED from disk ($0); Origami dropped from run one; $0-first pipeline planned

**What.** Investigation + plan only (nothing executed/purchased). After Murdoch rejected the run-one plan's economics (~1,000 credits → only ~55–100 contractors; $150–300 append too expensive), rethought the pipeline around owned assets. Full plan: Notion child page "Cheap ABM Rethink — $0 Recovered-List Pipeline [2026-07-09]" (id `39832498-a1cb-81b8-9192-ce0a85f73604`) under Distribution & Partnerships; run-one plan page marked superseded-in-part.

**Job 1 — leads recovered ($0).** The May–June leadgen runs (Prospeo→Instantly, connector-to-connector) ran in the desktop app's local agent mode; their transcripts persist under `AppData\Roaming\Claude\local-agent-mode-sessions`. Parsing the logged `add_leads_to_campaign_or_list_bulk` payloads (171 calls, 8 files) recovered **1,059 distinct leads** — 100% with first/last name + company + work email; 1,042 distinct companies; all 10 campaigns incl. Pressure Washing (`11aade60-…`). **0% personal-domain emails** (all company-domain work emails); no geo captured (Prospeo tool results not retained in transcripts). Extracted CSV kept scratch-only and deleted at session end (re-derivable in ~1 min from the persistent transcripts).

**Job 2 — recovery pricing.** Instantly 402 re-verified live on `workspace_get`/`list_lead_lists`. Cheapest documented pay-to-export: **one month Growth $47** (export gated on active-subscription not tier; data survives cancellation per help.instantly.ai 13924078/6285719); $9 Nano Credits plan possibly qualifies but unverified. Prospeo as recovery route: dead (no history/export API; post-downgrade retention undocumented). Re-sourcing via Origami ≈ 4,200+ credits (> balance). Disk recovery at $0 makes all of it moot unless the recovered set proves incomplete.

**Job 3 — recommended architecture ("v0 free, escalate on evidence").** Phase 0 ($0): Places geo-join (Text Search **Pro SKU 5,000 free calls/mo** — pilot spent only Enterprise SKU) → upload 1,059 stacked rows (EMAIL+FN+LN+CT+ST+ZIP+COUNTRY) to NEW audience "ABM v0" → probes (delivery_status, free 1%-LAL attempt = binary ≥100-matched readout, ad-set est size) + **Prospeo free test: 10 mobile lookups** (100 free credits, 10cr/found-only, name+company+website input — no LinkedIn needed, `only_verified_mobile`). Expected v0 match: ~50–250 (work-email band 2–20% + name/geo stacking). GATE A → optional Phase 1 (~$49–99, one Prospeo month, cancel after): bulk mobiles on all 1,059 at hit-rate-scaled cost ($0.10–0.245/found mobile), re-upload with PHONE → plausibly 300–500+ matched. GATE B → ~$30–60 auction Reach ad + Pixel retarget (spec unchanged from run-one plan). **Origami dropped from run one**: sourcing unneeded (names owned), 3cr emails would re-buy work emails (same vendor waterfall class), 15cr phones dominated by Prospeo 10cr; 2,997.75-credit balance banked for future new-segment sourcing. Fallbacks: Datazapp reverse-email append ($125 balance floor, ~$30 consumed, free match report first), Instantly $47 export month.

### 2026-07-09 [Source: Claude Code] — Origami → Meta Custom Audience execution plan (run one) researched + saved to Notion; PLAN ONLY, nothing executed

**What.** Researched and planned the Origami → Meta customer-list pipeline (goal set by the [Source: claude.ai] "Distribution Engine Plan (ABM)" Notion page). Full plan saved as a [Source: Claude Code] child page under Distribution & Partnerships: "Origami → Meta Custom Audience — Execution Plan (run one) [2026-07-09]" (Notion id `39832498-a1cb-81ce-b3ec-db56ae468930`). **0 Origami credits spent** (free reads only), no Meta object created, no ad, no outreach.

**Environment verified live.** Origami key authenticates, balance **2,997.75 credits** (`GET /api/v2/account/credits`); Meta Ads MCP live on ad account `978213371900828`; pilot audience `120254629684780273` reads ACTIVE with `approximate_count` bounds pinned 1000/1000 (still below display floor at 4,301 uploaded); Meta Pixel `1500154638449582` live at `app/layout.tsx:16`; Prospeo MCP on FREE plan, 100 credits (QA fallback).

**Load-bearing research findings (primary sources, cited in the Notion page).** (1) Origami official pricing confirmed: verified email = 3 credits, verified phone = 15 credits, free if not found ([origami.chat/pricing](https://origami.chat/pricing)) — so an FB-matchable contractor (owner personal mobile + email) costs ~18.75 credits when both hit / ~10 expected, and **~1,000 credits buys ~55–100 enriched contractors, not ~1,300** (the ~0.75 cr/business smoke-test rate priced un-enriched listing data — the class that failed the pilot). (2) Origami's personal-mobile capability is marketed (9-provider waterfall, carrier-verified, mobile-vs-direct-dial flag) but has zero published hit rates and zero independent reviews; their own blog admits "Limited phone data — Email-focused currently" → calibration gate required. (3) Meta multi-key stacking officially recommended; MCP upload tool hashes raw PII server-side (SHA-256). (4) No documented hard delivery minimum for customer-list audiences — 1,000 matched is a recommendation, 100 is only the lookalike-seed minimum; ads can run against hidden-size audiences (undocumented too-small INACTIVE threshold = residual risk). (5) Match benchmarks: consumer identifiers 60–80% vs B2B 2–30% (pilot's <25% fits); stacking lifts ~50%→~80% in the one published A/B. (6) Datazapp/Versium consumer append is ~10x cheaper per record at scale ($0.02–0.075/match, address-input) — flagged as the scale path if run one's economics hold.

**Plan shape (execution pending Murdoch's approval).** Stage A calibration: 25 owner-identified lawn-care/landscaping contractors, DFW, ~260–490 cr → measure phone hit rate, mobile-vs-listed-landline distinctness, email mix, true credits/contractor → GATE A. Stage B fill to ~950 cr total (~75–100 rows, ~40–60 with personal mobile). Upload to a NEW audience "ABM v1" (`PARTNER_PROVIDED_ONLY`, schema EMAIL+PHONE+FN+LN+CT+ST+ZIP+COUNTRY, ≤570-row batches) — expected matched **~30–75, below the 1,000 display floor (accepted)**; mitigation for the later Reach ad: target ABM v1 ∪ pilot audience. GATE B before any ad spend. Honest read: ~1,000 credits is right-sized for learning, undersized for the audience itself; scale path is Places-repull + consumer append (~$150–300 to clear 1,000 matched), not more Origami credits.
