...
## Step-2 landing video: native iPhone styling via iOS status bar + home indicator CSS overlays — 2026-05-13 [Source: Claude Code]

`public/videos/landing/step-2.mp4` recropped again from 978×1536 → **978×1448** (`crop=978:1448:0:336`) and the `PhoneFrame` component in [`app/(public)/page.tsx`](../app/%28public%29/page.tsx) extended with a `variant?: "default" | "web"` prop. When `variant === "web"` (set only on step-2's `STEPS` entry; steps 1/3/4 unchanged), the phone frame renders a synthetic iOS status bar overlay (`absolute inset-x-0 top-0 z-10 h-[28px] bg-white`, inline SVGs for the 4-bar signal / 3-arc wifi / battery glyphs at right, "9:41" left in SF Pro / system-ui semibold at 11/12px, black on white to match the form's white background and read as continuous chrome) and a small dark home indicator pill (`absolute bottom-[6px] h-[3.5px] w-[88px] bg-[#0B0E14]/85` lg:`w-[96px]`). The tighter crop leaves ~80 px residual top + ~40 px residual bottom whitespace at full resolution, which at the phone-frame's `object-cover` scale (504/1448 = 0.348) maps to ~28 px / ~14 px — exactly the heights the two overlays cover, so the form starts immediately below the status bar with no visible empty white in most frames. Side cropping increases slightly from 12.6%/side at the previous AR 0.637 to 14.7%/side at the new AR 0.6754 (accepted tradeoff — the form is centered horizontally with ample margin). Steps 1/3/4 have no `variant` prop, default to `default`, and render byte-identically to before. `npx tsc --noEmit` clean.
