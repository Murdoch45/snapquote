...
## Origami AI lead-sourcing integration installed + smoke-tested (Claude Code skills + API key) — 2026-07-09 [Source: Claude Code]

Origami ([origami.chat](https://origami.chat)) — an AI lead-sourcing / prospecting SaaS (one natural-language prompt → enriched lead list from Google Maps + web) — is wired into the **local Claude Code environment** as a **skills + API-key** integration, **NOT an MCP server**. The official installer (`https://origami.chat/skills/install.sh`, read in full before running — pure POSIX `set -eu`, only downloads a manifest and writes `SKILL.md` files, no code execution / no shell-rc edits / no secret exfil) placed **5 skills** into the **global** Claude Code skills dir `~/.claude/skills/`: `origami-api`, `origami-list-building`, `origami-sequencer`, `origami-scheduled-agents`, `origami-webhooks` — kept out of the SnapQuote repo deliberately (installed with the explicit `claude-global` target; the non-interactive default would otherwise seed every host). The API key lives in the project-root `.env` as `ORIGAMI_API_KEY=og_live_…` — gitignored (`git check-ignore .env` matches `*.env`; `git status` never lists it; never committed). **Live smoke test (2026-07-09 ~13:57 PT):** `GET /api/v2/account/credits` → HTTP 200 (auth OK, balance 3000); `POST /api/v2/agents` "Find 3 lawn care businesses in Los Angeles" → built table `e028d0b4-b6a1-4b41-817f-423ae03cbb2b` with **3 real enriched rows** (Evergreen Landcare, Flores Artscape, Angel's Lawn & Tree Service — E.164 phones, websites, addresses, Google/Yelp ratings, emails), independently re-read via the free data plane (`GET /api/v2/tables/:id/rows`, `total=3`). Cost **2.25 credits** (3000 → 2997.75). The run's terminal status was `incomplete` (a known Origami quirk where the agent's final tool-call framing doesn't parse) **but the table materialized fully** (leadCount 3, 0 cells running/errored). API base `https://origami.chat`, auth `Authorization: Bearer $ORIGAMI_API_KEY`. Skills auto-load for future agent chats after a Claude Code restart; this session called the API directly per the skill contract, so no restart was needed to verify. **Setup + smoke test only** — no bulk pull, enrichment, or Meta upload. Full run detail in [`docs/updates-log.md`](updates-log.md) under 2026-07-09.

## Facebook match-rate pilot (lawn care + landscaping) — audience live, matched below Meta's ~1,000 threshold — 2026-07-09 [Source: Claude Code]

Meta Custom Audience "Pilot - Lawn Care LA/MIA/DAL" (id `120254629684780273`) on Business ad account `978213371900828` (portfolio `1991446654777997`) now holds **both trades**: 1,701 lawn-care + 2,600 landscaping records = **4,301 raw rows uploaded** (phone + city + state + zip + country), distinct somewhat lower after Meta's automatic dedup of the lawn-care/landscaping phone overlap (the two trades were pulled separately across LA/Miami/Dallas via Google Places Text Search (New); the prior lawn-care phone list had been deleted as PII, so landscaping was deduped within itself and overlap left to Meta). Live `ads_get_custom_audience` (2026-07-09 ~10:20 PT, `operation_status "Normal"`): `approximate_count` **1,000/1,000** — matched size is **below Meta's ~1,000 privacy display floor even at ~4,300 uploaded**, so the blended match rate is **under ~25%** and the exact figure is hidden by Meta. (Lawn-care-only, at 1,701 uploaded, was likewise below the floor → then only bounded as <~59%; adding landscaping to ~4,300 tightened the bound to <~25%.) Audience only — no campaign/ad set/ad, **$0 ad spend**. Google Places integration on GCP project `631878227336` is functional (billing + "Places API (New)" enabled); ~782 lawn-care + 288 landscaping ≈ **~1,070 billable Text Search calls this month** — ~70 past the 1,000 free Enterprise-SKU tier ≈ **~$2.45** Places spend. Raw lists were scratch-only (PII), not committed. Full run detail in [`docs/updates-log.md`](updates-log.md) under 2026-07-09.
...
A new passive [`components/OutOfCreditsBanner.tsx`](../components/OutOfCreditsBanner.tsx) renders at the top of `/app/*` (via [`app/app/layout.tsx`](../app/app/layout.tsx)) and on `/dashboard/my-link` when both monthly and bonus credit pools are zero. Copy: "You're out of credits." + "You need credits to unlock new leads and send estimates." Plan-aware CTA: SOLO/TEAM → "Upgrade" → `/app/plan`; BUSINESS → "Buy more credits" → `/app/credits`. Coexists with the existing point-of-action `OutOfCreditsModal` ([`components/OutOfCreditsModal.tsx`](../components/OutOfCreditsModal.tsx)) at unlock time — banner = passive heads-up, modal = at-action interrupter; both point at the same recovery surfaces.
