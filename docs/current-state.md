...
- **Supabase Auth URL Configuration has leading whitespace in Site URL (and possibly Redirect URLs)** — confirmed live: every row in `auth.flow_state` for OAuth attempts (Google + Apple from May 1, 2026 17:36 onward) has `referrer = " https://snapquote.us"` with first char ASCII 32 (space), length 21. GoTrue persists this from the Site URL config at /authorize time and parses it on /callback; Go's `url.Parse` rejects strings with leading whitespace because `https` becomes a path segment that "cannot contain colon", and GoTrue 500s. **This is the actual cause of the Cowork-reported 500 on Google Sign-In** (and would also break Apple OAuth on web). Fix in Supabase Studio → Authentication → URL Configuration:
  - Edit "Site URL". Triple-click to select all, retype as `https://snapquote.us` (no leading/trailing whitespace, no surrounding quotes). Save.
  - Edit "Redirect URLs" allowlist. For each entry, retype to ensure no leading/trailing whitespace. Should be: `https://snapquote.us/auth/callback`, `https://snapquote.us/**` (or whichever wildcard pattern covers our routes), and `snapquotemobile://*` (mobile).
  - After saving, run: `select id, '|' || COALESCE(referrer, '') || '|' as referrer FROM auth.flow_state ORDER BY created_at DESC LIMIT 1;` — issue a fresh OAuth attempt and confirm the new flow_state row's referrer is `|https://snapquote.us|` (no space, length 20 instead of 21).
- **Supabase Google OAuth provider not enabled** — Web Google sign-in button is wired correctly; mobile is now PKCE-correct as of this session. Provider just isn't enabled at the project level. Live verification: `auth.identities` has 90 `email` rows and 1 `google` row (developer's own account, never reused over 3 weeks). Fix in Supabase Studio → Authentication → Providers → Google → Enable. Provide Client ID + Client Secret from Google Cloud Console (Cloud project `631878227336`'s OAuth credentials). Verify allowed redirect URLs include `https://upqvbdldoyiqqshxquxa.supabase.co/auth/v1/callback`. **AND** ensure the Redirect URLs whitespace fix above is done in the same Studio session. Without the whitespace fix, the OAuth /callback will continue to 500 even after Google is enabled.
