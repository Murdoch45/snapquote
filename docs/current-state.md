...
## Step-2 landing video recropped to match steps 1/3/4 aspect ratio — 2026-05-13 [Source: Claude Code]

`public/videos/landing/step-2.mp4` recropped from 978×1328 (AR 0.736) to **978×1536 (AR 0.637)** so it sits in the phone frame at the same side-crop ratio as steps 1/3/4 (12.6%/side instead of 17.6%/side). The source was the raw Canva-exported `Step 2.mp4` (978×2116), cropped via `ffmpeg-static` with `crop=978:1536:0:306 -c:v libx264 -crf 23 -preset medium -pix_fmt yuv420p -movflags +faststart -an` (drops 580 px total: 306 top + 274 bottom, leaving ~110 px residual top whitespace + ~98 px residual bottom whitespace). Tradeoff vs the previous tight crop: AR now matches the other steps but ~7% top + ~6% bottom of the in-phone-frame display shows as thin white strips because the cropped video isn't fully edge-to-edge content. No CSS change needed — the `PhoneFrame` in [`app/(public)/page.tsx`](../app/%28public%29/page.tsx) `object-cover object-center` handles the new AR identically to the others. Originally attempted via fal.ai Luma Ray 2 Flash Reframe at aspect_ratio=9:16 ($0.96 charged on job 019e2280-fc99-7440-a13a-026655847186) but the model hallucinated fake mirrored UI panels on the right side and bottom of every frame — see entry in [`docs/updates-log.md`](updates-log.md) for the full reasoning on why outpaint failed for a UI screen recording and what to try next time. `npx tsc --noEmit` clean.
