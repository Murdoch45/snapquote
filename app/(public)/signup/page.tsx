...
import { parseServiceTypesParam } from "@/lib/services";

type Props = {
  searchParams?: Promise<{ onboarding?: string; workTypes?: string }>;
};

export default async function SignupPage({ searchParams }: Props) {
  const params = await searchParams;
  const onboarding = params?.onboarding === "1";
  const initialServices = parseServiceTypesParam(params?.workTypes);
    ...
    <main className="flex min-h-screen items-center justify-center bg-gray-50 px-4 py-10">
      <div className="w-full max-w-lg space-y-4">
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-6">
            {onboarding ? (
              <FinishOnboardingForm initialServices={initialServices} />
            ) : (
