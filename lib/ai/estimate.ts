...
import {
  ...
  serviceQuestions,
  type FormattedServiceQuestionAnswer,
...
const aiSignalsSchema = z.object({
  // Top-level summary is no longer generated by the estimator signal call —
  // it's produced deterministically from questionnaire answers after the
  // engine runs, then polished by a separate, narrowly-scoped AI call.
  // Accept it on legacy / cached replay inputs but don't require it.
...
function buildSignalPrompt(
  ...
  return [
    ...
    "Do NOT produce a top-level summary. The job summary is assembled deterministically from questionnaire answers by backend code after you return. Focus on the structured signals.",
    JSON.stringify(
      {
...
export function fallbackEstimate(
  ...
  const deterministicSummary = buildDeterministicJobSummary(input);
  return buildGeneratedEstimate(
    engineEstimate,
    propertyData,
    finalSignals,
    estimatorAudit,
...
// Label keywords we want in sentence 1 (what the customer wants done) vs
// sentence 2 (how the site looks). Matched against the lowercased formatted
// label string. Anything not matched goes into the sentence-2 bucket.
const SCOPE_LABEL_KEYWORDS = [
  "work",
  "need help",
  "job type",
  "project",
...
const SIZE_LABEL_KEYWORDS = ["size", "area", "count", "length", "quantity", "square", "linear"];
const MATERIAL_LABEL_KEYWORDS = ["material", "surface", "tier"];
const CONTEXT_LABEL_KEYWORDS = [
  "access",
  "difficulty",
  "condition",
  "property",
...
function matchesAnyKeyword(label: string, keywords: readonly string[]): boolean {
  const lower = label.toLowerCase();
  return keywords.some((keyword) => lower.includes(keyword));
}

function formatAnswerClause(answer: FormattedServiceQuestionAnswer): string {
  const label = answer.label.replace(/:\s*$/, "").trim();
  const value = answer.value.trim();
  if (!value) return "";
  return `${label.toLowerCase()} is ${value.toLowerCase()}`;
}

function buildSummarySentences(
  services: string[],
  bundles: ServiceQuestionAnswerBundle[]
): { scopeSentence: string; contextSentence: string } {
  const serviceNames = (services.length > 0 ? services : bundles.map((b) => b.service)).filter(Boolean);
  const serviceLabel =
  ...
  // Collect formatted answers across every bundle that matches a declared
  // service. Deliberately skip bundles whose service isn't in the declared
  // list — that's the guardrail against off-topic summaries (e.g. fence
  // answers leaking into a concrete job).
  const declaredServices = new Set(serviceNames);
  ...
  for (const bundle of bundles) {
    if (declaredServices.size > 0 && !declaredServices.has(bundle.service)) continue;
    const formatted = formatServiceQuestionAnswers(bundle.service, bundle.answers);
    for (const answer of formatted) {
      const clause = formatAnswerClause(answer);
      if (!clause) continue;
      if (matchesAnyKeyword(answer.label, SCOPE_LABEL_KEYWORDS)) {
        scopeParts.push(clause);
      } else if (matchesAnyKeyword(answer.label, SIZE_LABEL_KEYWORDS)) {
        sizeParts.push(clause);
      } else if (matchesAnyKeyword(answer.label, MATERIAL_LABEL_KEYWORDS)) {
        materialParts.push(clause);
      } else if (matchesAnyKeyword(answer.label, CONTEXT_LABEL_KEYWORDS)) {
        contextParts.push(clause);
      } else {
        contextParts.push(clause);
      }
    }
  ...
  const scopeBits = [...scopeParts, ...sizeParts, ...materialParts].slice(0, 4);
  const scopeSentence =
    scopeBits.length > 0
      ? `The customer requested ${serviceLabel}: ${scopeBits.join("; ")}.`
      : `The customer requested ${serviceLabel}.`;

  const contextBits = contextParts.slice(0, 3);
  const contextSentence =
    contextBits.length > 0
      ? `Based on the questionnaire, ${contextBits.join("; ")}.`
      : "No additional site details were provided.";

  return { scopeSentence, contextSentence };
}

export function buildDeterministicJobSummary(input: EstimateInput): string {
  const bundles = input.serviceQuestionAnswers ?? [];
  const { scopeSentence, contextSentence } = buildSummarySentences(input.services, bundles);
  return `${scopeSentence} ${contextSentence}`.slice(0, 600);
}
              ...
              content: [
                {
                  type: "input_text",
                  text:
                    "You polish SnapQuote job summaries for contractors. Rewrite the provided factual summary in natural, plain-English contractor-briefing voice. Keep it to exactly two sentences. Do not add any facts, numbers, services, materials, or context that aren't already in the input. Do not remove facts. No bullet points, colons, or markdown. Respond with only the rewritten summary, nothing else."
                }
              ]
            },
            {
